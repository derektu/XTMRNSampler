...
import { MockServerHost } from './MockServerHost';
...
class RTChannel {  
  ...
  ref(symbolId) {
    ...
    // DEBUG
    if (this.refs.has(symbolId.toLowerCase())) {
      console.warn(`RTChannel.ref() duplicate symbolId:${symbolId}`);
      return;
    }
    //
  ...
  unref(symbolId) {
    // DEBUG
    if (!this.refs.has(symbolId.toLowerCase())) {
      console.warn(`RTChannel.unref() non-existing symbolId:${symbolId}`);
      return;
    }
    //
...
export class RTSvc {
  ...
  _updateQuotes() {
    ...
    this._getQuotes(symbolIds, (err, rtqs)=> {
      if (err) {
        ...
        _.each(rtqs, (rtquote)=> {
          // update quote data
          this.quotes[rtquote.symbolId.toLowerCase()] = rtquote;
  ...
  _getQuotes(symbolIds, cb) {
    ...
    let url = `${MockServerHost}/xtmrnserver/api/rtquote?id=${baseIds.join(',')}`;
