...
const StockCellRow = (props)=> {
  ...
  let cells = stocks.slice(0, 5).map(stock=> {
    ...
    return <><HSpacing size='4'/><StockCell symbolId={symbolId} key={symbolId}/></>
  }) 

  return (
    <View style={styles.stockCellRowContainer}>
      <VSpacing size='4'/>
...
class StockWidget extends React.Component {
  ...
  render() {
    ...
    if (isLoading) {
      ...
      body = <StockCellRow style={styles.stockCellRowContainer} stocks={stocks}/>
    }
